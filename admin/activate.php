<?php
	...
	if(is_dir('../install')) {
		...
		if((isset($_GET['adminEmail']) && !empty($_GET['adminEmail'])) && (isset($_GET['hash']) && !empty($_GET['hash']))) {
			// Set some variables
			$adminEmail = $mysqli->real_escape_string($_GET['adminEmail']);
			...
			$check2 = $mysqli->query("SELECT 'X' FROM admins WHERE adminEmail = '".$adminEmail."' AND hash = '".$hash."' AND isActive = 1");
			if ($check2->num_rows) {
				$activeAccount = 'true';
			}

			// Match found, update the Manager's account to active
			if ($match > 0) {
				$isActive = '1';
								...
								<div class='login'>
									...
									?>
											...
											<div class="alertMsg success no-margin">
												<i class="fa fa-check"></i> <?php echo $activateMsg1; ?>
											</div>
