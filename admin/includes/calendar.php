		...
		$('#calendar').fullCalendar({
			...
			<?php
				...
				$query = "SELECT
						...
						WHERE
							adminevents.isPublic = 0 AND
							adminevents.adminId = ".$adminId;
				...
				while($row = mysqli_fetch_assoc($res)) {
					...
					if ($row['isPublic'] == '1') { $isPublic = '<small class="label label-primary preview-label">'.$pulicEvent.'</small>'; } else { $isPublic = ''; }
				...
				$stmt = "SELECT
						...
						WHERE
							clientevents.isShared = 1";
				$results = mysqli_query($mysqli, $stmt) or die('-3'.mysqli_error());

				while($col = mysqli_fetch_assoc($results)) {

					// Months start at 0 - so subtract 1 month from the query dates
					$eventStartMnth = $col['startMonth'];
					$eventStartMonth = --$eventStartMnth;
					$eventDate = strtotime($col['startDate']);
					$eventDate = date('Y', $eventDate).', '.$eventStartMonth.', '.date('d, H, i', $eventDate);

					$eventEndMnth = $col['endMonth'];
					$eventEndMonth = --$eventEndMnth;
					$eventEnd = strtotime($col['endDate']);
					$eventEnd = date('Y', $eventEnd).', '.$eventEndMonth.', '.date('d, H, i', $eventEnd);

					// Check for an All Day event
					if ($col['timeStart'] != '00:00') { $allDay = 'allDay: false,'; } else { $allDay = 'allDay: true,'; }
					if ($col['startTime'] == '00:00') { $startTime = ''; } else { $startTime = $col['startTime']; }
					if ($col['endTime'] == '00:00') { $endTime = ''; } else { $endTime = $col['endTime']; }
					// Check for an End Date
					if ($col['dateEnd'] != '0000, 00, 00') { $endsOn = "end: new Date(".$eventEnd."),"; } else { $endsOn = ""; }
					// Set the Times to Display
					if ($col['displaystart'] != '12:00 AM') { $displaytime = $col['displaystart'].' &mdash; '.$col['displayend']; } else { $displaytime = $noTimesSet; }
					// Check if it is a Shared Event
					if ($col['isShared'] == '1') { $isShared = '<small class="label label-info preview-label">'.$sharedEvent.'</small>'; } else { $isShared = ''; }

					echo $delim."{";
					echo "
							title: '".$col['eventTitle']."',
							start: new Date(".$eventDate."),
							startsondate: '".$col['startsOnDate']."',
							endsondate: '".$col['endsOnDate']."',
							starttime: '".$startTime."',
							endtime: '".$endTime."',
							".$endsOn."
							".$allDay."
							color: '".$col['eventColor']."',
							desc: '".$col['eventDesc']."',
							startson: '".$col['displayDate']."',
							displaytime: '".$displaytime."',
							shared: '".$isShared."',
							isPublic: '',
							colorinput: '".$col['eventColor']."',
							postedby: '".$col['postedBy']."',
							eventid: '".$col['clienteventId']."',
							uid: '".$col['clientId']."',
							isclient: '1'
						";
					echo "}";
			...
			eventClick: function(calEvent, jsEvent, view) {
				...
				if (calEvent.isPublic !== '') {
					$('.event-desc').show().html(calEvent.isPublic + ' <span class="label label-default preview-label">' + calEvent.displaytime + '</span><span class="pull-right"><span class="label label-default preview-label"><?php echo $eventPostedBy; ?>: ' + calEvent.postedby + '</span></span><br /><br />' + calEvent.desc.replace(/\r\n/g, "<br />"));
				} else if (calEvent.isShared !== '') {
					$('.event-desc').show().html(calEvent.shared + ' <span class="label label-default preview-label">' + calEvent.displaytime + '</span><span class="pull-right"><span class="label label-default preview-label"><?php echo $eventPostedBy; ?>: ' + calEvent.postedby + '</span></span><br /><br />' + calEvent.desc.replace(/\r\n/g, "<br />"));
				} else {
					$('.event-desc').show().html('<span class="label label-default preview-label">' + calEvent.displaytime + '</span><span class="pull-right"><span class="label label-default preview-label"><?php echo $eventPostedBy; ?>: ' + calEvent.postedby + '</span></span><br /><br />' + calEvent.desc.replace(/\r\n/g, "<br />"));
				}
				...
				$('.isclient').val(calEvent.isclient);
