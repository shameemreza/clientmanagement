...
/*
 * pagination.php
 * Used on pages with the $action & an ID passed in the URL
 */
class paginator {
	/**
	...
	*/
	public function page_links($path='&', $ext=null) {
		...
		if ($this->_page > 1) {
			$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=$prev"."$ext'><i class='fa fa-angle-double-left'></i> ".$previousLink."</a>";
		} else {
		...
		if ($lastpage < 7 + ($adjacents * 2)) {
			for ($counter = 1; $counter <= $lastpage; $counter++) {
				if ($counter == $this->_page) {
					...
					$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=$counter"."$ext'>$counter</a>";
				}
			}
			...
			if ($this->_page < 1 + ($adjacents * 2)) {
				...
				$pagination.= "&hellip;";
				$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=$lpm1"."$ext'>$lpm1</a>";
				$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=$lastpage"."$ext'>$lastpage</a>";
			} else if ($lastpage - ($adjacents * 2) > $this->_page && $this->_page > ($adjacents * 2)) {
				$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=1"."$ext'>1</a>";
				$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=2"."$ext'>2</a>";
				$pagination.= "&hellip;";
		...
		if ($this->_page < $counter - 1) {
			$pagination.= "<a href='index.php?action=".$_GET['action'].'&'.$getId.$path."$this->_instance=$next"."$ext'>".$nextLink." <i class='fa fa-angle-double-right'></i></a>";
			$pagination.= "</div>\n";
		} else {
