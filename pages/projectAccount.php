<?php
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == 'editEntry') {
		...
		if($_POST['entryTitle'] == "") {
			...
			$stmt->execute();
			$msgBox = alertBox($accountEntryUpdated, "<i class='fa fa-check-square'></i>", "success");
			$stmt->close();
		}
	}
	...
	// Only allow access to the Client
	if ($totalRecs > 0) {
		$qry = "SELECT
					clients.clientId
				FROM
				...
				WHERE clientprojects.projectId = ".$row['projectId'];
...
<?php } else { ?>
	<div class="content">
		...
		<div class="row">
			<div class="col-md-6">
				<table class="infoTable">
					<tr>
						<td class="infoKey"><i class="fa fa-quote-left"></i> <?php echo $entryTitleText; ?>:</td>
		...
		<div class="well well-sm bg-trans no-margin mt20">
			<strong><?php echo $descText; ?>:</strong> <?php echo nl2br($entryDesc); ?>
		</div>

		<?php if ($entryNotes != '') { ?>
			<div class="well well-sm bg-trans no-margin mt20">
				<strong><?php echo $notesField; ?>:</strong> <?php echo nl2br($entryNotes); ?>
			</div>
		<?php } ?>

		<a data-toggle="modal" data-target="#editEntry" class="btn btn-success btn-icon mt20"><i class="fa fa-edit"></i> <?php echo $editEntryBtn; ?></a>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-header">
						...
						<h4 class="modal-title"><?php echo $editEntryBtn; ?></h4>
					</div>
					...
					<form action="" method="post">
						...
						<div class="modal-footer">
							<button type="input" name="submit" value="editEntry" class="btn btn-success btn-icon"><i class="fa fa-check-square-o"></i> <?php echo $updateEntryBtn; ?></button>
